...
class StartAssessmentRecapExport implements FromCollection, Responsable, WithCustomStartCell, WithMapping, ShouldAutoSize, WithStyles, WithTitle
{
    use Exportable;
    ...
    public function map($user): array
    {
        ...
        if ($assessments->count() > 0) {
            foreach ($assessments as $assessment) {
                $score = 0;
                $score += $assessment->presentation_file_pillar_two * 0.25;
                $score += $assessment->presentation_file_pillar_one * 0.25;
                $score += $assessment->presentation_file_pillar_three * 0.2;
                $score += $assessment->presentation_file_pillar_four * 0.15;
                $score += $assessment->presentation_file_pillar_five * 0.15;
