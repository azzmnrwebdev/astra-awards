<x-admin title="Induk Perusahaan">
    {{-- Main Content --}}
    <h4 class="mb-4 fw-semibold d-inline-flex">Manajemen Induk Perusahaan</h4>
        ...
        <div class="card-body p-lg-4">
            ...
            <div class="row align-items-center">
                <div class="col-sm-6 col-xl-8">
                    <a href="{{ route('parent_company.create') }}" class="btn btn-dark rounded-0">Tambah</a>
                </div>
            ...
            <div class="table-responsive mt-4">
                <table class="table table-hover text-nowrap align-middle mb-0">
                    ...
                    <tbody class="border-start border-end">
                        @forelse ($parentCompanies as $item)
                            <tr>
                                <td class="text-center py-3">{{ $loop->index + $parentCompanies->firstItem() }}</td>
                                <td class="text-start py-3">{{ $item->name }}</td>
                                <td class="text-center py-3">
                                    <a href="{{ route('parent_company.edit', ['parentCompany' => $item->id]) }}"
                                        class="text-dark align-middle @if (empty(count($item->company))) me-3 @endif"><i
                                            class="bi bi-pencil"></i></a>

                                    @if (empty(count($item->company)))
                                        <button type="button"
            ...
            {{-- Pagination --}}
            <div class="mt-3">
                {{ $parentCompanies->appends(request()->query())->links() }}
            </div>
        </div>
    </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                function filter() {
                    ...
                    const url = '{{ route('parent_company.index') }}';
                ...
                $('.delete').click(function() {
                    ...
                    const deleteUrl = "{{ route('parent_company.destroy', ['parentCompany' => ':id']) }}"
                        .replace(':id',
                            id);
