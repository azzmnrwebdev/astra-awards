<x-admin title="Edit {{ $company->name }}">
    {{-- Custom CSS --}}
    @prepend('styles')
        <style>
            #pageTitle:hover {
    ...
    <h4 class="mb-4 fw-semibold d-inline-flex" id="pageTitle">
        <i class="bi bi-arrow-left-short" style="-webkit-text-stroke: 1px;"></i>&nbsp;&nbsp;Edit {{ $company->name }}
    </h4>
                        ...
                        <select name="parent_company_id" id="parent_company_id"
                            ...
                            @foreach ($parentCompanies as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('parent_company_id', $company->parent_company_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                    ...
                    <div class="col-md-9 col-xl-10">
                        <select name="business_line_id" id="business_line_id"
                            ...
                            @foreach ($businessLines as $item)
                                <option value="{{ $item->id }}"
                                    {{ old('business_line_id', $company->business_line_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
