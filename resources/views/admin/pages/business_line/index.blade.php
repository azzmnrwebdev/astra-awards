        ...
        <div class="card-body p-lg-4">
            ...
            <div class="row align-items-center">
                <div class="col-sm-6 col-xl-8">
                    <a href="{{ route('business_line.create') }}" class="btn btn-dark rounded-0">Tambah</a>
                </div>

                <div class="col-sm-6 col-xl-4 mt-3 mt-sm-0">
                    <form>
                        <input type="search" name="pencarian" id="pencarian" value="{{ $search }}"
                            class="form-control" placeholder="Cari lini bisnis?">
                    </form>
                </div>
            </div>
            ...
            <div class="table-responsive mt-4">
                <table class="table table-hover text-nowrap align-middle mb-0">
                    ...
                    <tbody class="border-start border-end">
                        @forelse ($businessLines as $item)
                            <tr>
                                <td class="text-center py-3">{{ $loop->index + $businessLines->firstItem() }}</td>
                                <td class="text-start py-3">{{ $item->name }}</td>
                                <td class="text-center py-3">
                                    <a href="{{ route('business_line.edit', ['businessLine' => $item->id]) }}"
            ...
            {{-- Pagination --}}
            <div class="mt-3">
                {{ $businessLines->appends(request()->query())->links() }}
            </div>
        </div>
    </div>
        ...
        <script>
            $(document).ready(function() {
                ...
                function filter() {
                    ...
                    const url = '{{ route('business_line.index') }}';
                ...
                $('.delete').click(function() {
                    ...
                    const deleteUrl = "{{ route('business_line.destroy', ['businessLine' => ':id']) }}"
                        .replace(':id',
                            id);
